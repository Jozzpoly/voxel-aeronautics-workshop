...
- M3A - Reliable browser/UI smoke entrypoint is present as `npm run browser:smoke`: it starts a local static server, probes for a real Chromium/Chrome/Edge CDP target, checks starter-craft/UI/Flight Focus/contract-panel hit-testing, and reports browser/runtime gaps as `ENVIRONMENT`, not PASS. Next M3 work: decide where this target-platform evidence should run outside the default core gate.
