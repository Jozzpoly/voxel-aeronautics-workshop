...
assert.throws(() => Port.normalizeBodyDescriptor({ mass: NaN }), /finite number/);
assert.throws(() => Port.normalizeBodyDescriptor({ position: { x: Infinity, y: 0, z: 0 } }), /finite number/);
assert.throws(() => Port.normalizeBodyDescriptor({ quaternion: { x: 0, y: 0, z: 0, w: 0 } }), /non-zero length/);
assert.throws(() => Port.normalizeBodyDescriptor({ type: 'static', mass: 1 }), /Static body mass must be zero/);
assert.throws(() => Port.normalizeBodyDescriptor({ type: 'dynamic', mass: 0 }), /Dynamic body mass must be greater than zero/);
assert.throws(() => Port.normalizeWorldDescriptor({ solverIterations: 2.5 }), /positive integer/);
assert.throws(() => Port.normalizeWorldDescriptor({ solverTolerance: 0 }), /greater than zero/);
assert.throws(() => Port.normalizeMassProperties({ mass: 1, inertiaDiagonal: { x: -1, y: 1, z: 1 } }), /cannot be negative/);
assert.throws(() => Port.normalizeCollisionEvent({ impactSpeed: NaN }), /finite number/);
...
Physics.setBodyTransform(body, { axisAngle: { axis: { x: 2, y: 0, z: 0 }, angle: Math.PI } });
const axisAngleTransform = Physics.getBodyTransform(body);
assert(Math.abs(Math.abs(axisAngleTransform.quaternion.x) - 1) < 1e-9);
assert(Math.abs(axisAngleTransform.quaternion.w) < 1e-9);
Physics.setBodyTransform(body, { quaternion: { x: 0, y: 0, z: 0, w: 1 } });
assert.throws(() => Physics.vec3(NaN, 0, 0), /finite number/);
const savedX = body.position.x;
body.position.x = NaN;
assert.throws(() => Physics.getBodyTransform(body), /finite number/);
