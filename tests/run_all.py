...
def main() -> None:
    ...
    run('node', 'tests/test_vector_thruster_runtime_probe.js')
    run('node', 'tests/test_module_visual_cell_scale.js')
    ...
    run(sys.executable, 'tests/test_release_source_contract.py')
    ...
    run('node', '--check', 'tests/run_mobile_browser_smoke.mjs')
    run('node', '--check', 'tests/run_mobile_offline_release_smoke.mjs')
    ...
    run(sys.executable, 'tests/test_agent_dispatch.py')
