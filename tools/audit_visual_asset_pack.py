...
def audit_pack(pack_root: Path, manifest_name: str = DEFAULT_MANIFEST) -> dict:
    ...
    if not manifest_path.is_file():
        ...
        return {'ok': False, 'packRoot': str(pack_root), 'diagnostics': diagnostics, 'assets': []}
    ...
    for asset_index, asset in enumerate(assets):
        ...
        if not isinstance(block_types, list) or not block_types:
            diagnostics.append(diagnostic('error', 'visualAssetAudit.blockTypesMissing', f'{asset_path}.bindings.blockTypes', 'Asset must bind at least one block type.'))
            block_types = []
        for block_type in block_types:
            if block_type not in KNOWN_BLOCK_TYPES:
                diagnostics.append(diagnostic('error', 'visualAssetAudit.unknownBlockType', f'{asset_path}.bindings.blockTypes', f'Unknown block type: {block_type}'))
        ...
        if not is_safe_pack_path(model_path):
            diagnostics.append(diagnostic('error', 'visualAssetAudit.modelPathInvalid', f'{asset_path}.model.path', 'Model path must be a safe pack-relative path.'))
        else:
            try:
                model_file = resolve_pack_path(pack_root, model_path)
                model_exists = model_file.is_file()
                if not model_exists:
                    diagnostics.append(diagnostic('error', 'visualAssetAudit.modelMissing', f'{asset_path}.model.path', f'Model file does not exist: {model_path}'))
                else:
                    gltf_paths, gltf_names = node_records(load_json(model_file))
            except Exception as error:  # noqa: BLE001 - CLI diagnostic
                diagnostics.append(diagnostic('error', 'visualAssetAudit.modelInvalidJson', f'{asset_path}.model.path', str(error)))

        nodes = asset.get('bindings', {}).get('nodes') if isinstance(asset.get('bindings'), dict) else {}
        if not isinstance(nodes, dict):
            diagnostics.append(diagnostic('error', 'visualAssetAudit.nodesInvalid', f'{asset_path}.bindings.nodes', 'Node bindings must be an object.'))
            nodes = {}
        for alias in NODE_ALIASES:
            value = nodes.get(alias)
            if alias == 'visualRoot' and not (isinstance(value, str) and value.strip()):
                diagnostics.append(diagnostic('error', 'visualAssetAudit.visualRootMissing', f'{asset_path}.bindings.nodes.visualRoot', 'visualRoot is required.'))
            if isinstance(value, str) and value.strip() and model_exists:
                ok, reason = binding_resolves(value, gltf_paths, gltf_names)
                if not ok:
                    diagnostics.append(diagnostic('error', 'visualAssetAudit.nodeBindingUnresolved', f'{asset_path}.bindings.nodes.{alias}', f'{value}: {reason}'))
                if 'thuster' in value.lower():
                    diagnostics.append(diagnostic('info', 'visualAssetAudit.suspiciousBindingSpelling', f'{asset_path}.bindings.nodes.{alias}', f'Binding "{value}" contains "thuster"; review whether this is an intentional model node name or a carried-over typo.'))
            ...
            if value and allowed_types and any(block_type not in allowed_types for block_type in block_types):
                diagnostics.append(diagnostic('warning', 'visualAssetAudit.unexpectedRigBindingForBlockType', f'{asset_path}.bindings.nodes.{alias}', f'{alias} is unusual for {", ".join(block_types) or "unbound asset"}.'))
    ...
    return {'ok': ok, 'packRoot': str(pack_root), 'diagnostics': diagnostics, 'assets': audited_assets}
...
def main() -> int:
    ...
    report = audit_pack(args.pack_root, args.manifest)
