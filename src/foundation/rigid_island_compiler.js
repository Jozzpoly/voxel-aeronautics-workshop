(() => {
  ...
  window.VAW.define('foundation.rigid-island-compiler', [
    'foundation.catalog', 'foundation.mass-properties', 'foundation.diagnostics'
  ], (catalog, MassProperties, Diagnostics) => {
    ...
    function compile(structuralGraph, cutEdgeIds = []) {
      const diagnostics = []; const cuts = new Set(cutEdgeIds); const rigidAdjacency = Object.create(null);
      ...
      for (const edge of structuralGraph.edges) {
        ...
        rigidAdjacency[edge.blockAId].push(edge.blockBId); rigidAdjacency[edge.blockBId].push(edge.blockAId);
      }
      ...
      for (const block of structuralGraph.blocks) {
        ...
        const queue = [block.blockId]; visited.add(block.blockId); const ids = [];
        for (let cursor = 0; cursor < queue.length; cursor += 1) {
          const id = queue[cursor]; ids.push(id);
          for (const neighbor of rigidAdjacency[id]) if (!visited.has(neighbor)) { visited.add(neighbor); queue.push(neighbor); }
        }
        ids.sort(); components.push(ids);
      }
      ...
      const rigidIslands = []; const blockIdToBodyId = Object.create(null); const bodyById = Object.create(null); const parts = [];
      for (const blockIds of components) {
        ...
        const assemblyCenterOfMass = [...massProperties.centerOfMass];
        const role = coreId && blockIds.includes(coreId) ? 'root' : 'subassembly';
        const islandParts = [];
        for (const block of blocks) {
          const definition = BLOCKS[block.type];
          const bodyLocalPosition = sub(block.assemblyPosition, assemblyCenterOfMass);
          const fullForceBodyLocal = (block.type === 'Thruster' || block.type === 'VectorThruster')
            ? scale(block.basis.forward, Number(definition.force) || 0) : [0, 0, 0];
          const localTorqueBody = cross(bodyLocalPosition, fullForceBodyLocal);
          const part = {
            blockId: block.blockId, bodyId, gridKey: block.gridKey,
            assemblyPosition: [...block.assemblyPosition], bodyLocalPosition,
            type: block.type, orientation: block.orientation, controlAxis: block.controlAxis, controlSign: block.controlSign,
            basis: block.basis, mass: Number(definition.mass) || 0, fullForceBodyLocal, localTorqueBody,
            rigidNeighborBlockIds: [...rigidAdjacency[block.blockId]],
            properties: {
              force: Number(definition.force) || 0, fuelRate: Number(definition.fuelRate) || 0,
              fuelCapacity: Number(definition.fuelCapacity) || 0, wingArea: Number(definition.wingArea) || 0,
              dragArea: Number(definition.dragArea) || 0, durability: Number(definition.durability) || 60,
              structural: Number(definition.structural) || 0
            }
          };
          blockIdToBodyId[block.blockId] = bodyId; islandParts.push(part); parts.push(part);
        }
        const island = {
          bodyId, anchorBlockId, role, blockIds: [...blockIds],
          assemblyPose: { position: assemblyCenterOfMass, quaternion: [0, 0, 0, 1] },
          sourceAssemblyCenterOfMass: assemblyCenterOfMass,
          massProperties: { mass: massProperties.mass, centerOfMass: [0, 0, 0], inertiaDiagonal: [...massProperties.inertiaDiagonal] },
          parts: islandParts
        };
        rigidIslands.push(island); bodyById[bodyId] = island;
      }
      ...
      return deepFreeze({ rigidIslands, bodyById, rootBodyId, blockIdToBodyId, rigidAdjacencyByBlockId: rigidAdjacency, parts, diagnostics: Diagnostics.canonicalize(diagnostics) });
    }
